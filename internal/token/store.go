...
func (store *Store) Generate(ctx context.Context, accountId string, readOnly bool) (*Token, error) {
	t := &Token{}
	err := db.PgError(store.db.QueryRow(ctx, `INSERT INTO token (account_id, read_only) VALUES ($1, $2) RETURNING id, token, account_id, read_only;`,
		accountId, readOnly).Scan(&t.ID, &t.Token, &t.AccountID, &t.ReadOnly))
	return t, err
}

func (store *Store) Reroll(ctx context.Context, t *Token) error {
	err := db.PgError(store.db.QueryRow(ctx, `UPDATE token SET token = uuid_generate_v4() WHERE id = $1 AND account_id = $2 RETURNING id, token, account_id, read_only;`,
		t.ID, t.AccountID).Scan(&t.ID, &t.Token, &t.AccountID, &t.ReadOnly))
	return err
}

func (store *Store) Get(ctx context.Context, id string) (*Token, error) {
	t := &Token{}
	err := db.PgError(store.db.QueryRow(ctx, `SELECT id, account_id, read_only FROM token WHERE token = $1;`,
		id).Scan(&t.ID, &t.AccountID, &t.ReadOnly))
	return t, err
}
